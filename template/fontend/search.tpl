		...
		<div class="main-page span8">
			...
			{if isset($data_search)}
				{if $data_search <> 0}
					...
					<h2 class="about-header">Search&nbsp;&raquo;&nbsp;<a href="" style="font-size:16px;">{$data_search['keyword']}</a></h2>
					...
					{if $data_search['data'] != null}
						{foreach from = $data_search['data'] item = data}
							<header>
								<h2 class="page-header empty-title"><a href="{$base_url}/single.php?id={$data['post_id']}&amp;article={$data['post_title']|escape:'hex'}">{$data['post_title']}</a></h2>
